...
Data for this study were collected from the Demographic Health Survey (DHS) program, which is . We used the STATCompiler tool available from the DHS Program to develop customized tables on indicators related to  water, transportation, and wealth from 78 countries between 1990-2019.

We preprocessed the data by differentiating the different surveys collected by the DHS program between DHS and Malaria Indicators Survey (MIS) through excel. After, we used R to develop and simplify the table with the most recent surveys for the most up to date analysis for the 78 countries. There were two sets of data of population and households. Therefore, we decided to remove the population data from the table to simplify because population and households data were identical. 

We only preserved variables where less than 50\% of values were missing. Ultimately, simplified data frame was 76 countries by 28 variables. After, two different tables are generated where one focused on the water accessibility variables and a explanatory variables. From the remaining variables, we selected 11 which related to water accessibility.  These included: types of water source (piped into dwelling, public tap/standpipe, tube well/borehole, protected well, protected spring, rainwater, unprotected well, unprotected spring, surface water), and time to water source (less than 30 minutes, more than 30 minutes) \citep{price2019difference} \autoref{tab:summary-wa}. Remaining variables related to wealth and transportation were used as an explanatory variables in the model \autoref{tab:summary-var}. 
\begin{table}[h!]
  \centering
...
\\\midrule
 \texttt{lita} & Boat with a motor
 & 
\\\midrule
 \texttt{lity} & Boat with a motor
 & 
\\\midrule
 \texttt{mori} & Boat with a motor
 & 
\\\midrule
